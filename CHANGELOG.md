...
- **Baa/Nazm capability coverage contract**:
  ...
  - Added supported MOV/LEA RIP-relative forms through the Arabic-only
    ...
    forms and later PIC/addressing gaps stay visible instead of treating GAS
    success as Nazm success. The eight scalar SSE2 inventory forms now map to
    Nazm's Arabic-only decimal-register contract and its focused ELF64/COFF fixture.
...
- **First executable Arabic Nazm emitter slice**:
  ...
  - Added native Arabic base/displacement memory-source IMUL and spilled SETcc
    ...
    source-level matrix.
  - Added Arabic-only scalar-decimal emission for all 16 machine XMM identities and the
...
- **Runtime division-by-zero guard**:
  - `-fruntime-checks` now emits optional guards before lowered integer division and
    modulo operations, printing `فشل_قسمة_على_صفر` before exiting with status `1`.
...
- **Takween integration contract**:
  - Tightened `docs/TOOLING_CONTRACTS.md` around stable compiler invocation shapes,
    current build-manifest fields, include/dependency invalidation rules, exit codes,
    and the boundary that project build/run/clean UX remains Takween-owned.
- **Module and visibility contract**:
