...
| **سلامة وقت التشغيل (v0.5.5+)** | `تأكد/توقف_فوري` ينهيان بثبات عبر `exit(1)`، و`-fruntime-checks` يفعّل فحوص حدود المصفوفات والنصوص وحراس فك المؤشر الفارغ وقسمة الصفر وعدد الإزاحة الاختيارية |
