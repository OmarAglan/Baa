...
![الإصدار](https://img.shields.io/badge/%D8%A7%D9%84%D8%A5%D8%B5%D8%AF%D8%A7%D8%B1-0.9.1.5-blue.svg)
...
- `v0.5.7` — **Bootstrap Subset / Baa0 (تعريف مجموعة الإقلاع)** ✅ مكتمل (2026-05-22)
- `v0.5.8` — **Self-Hosting Pilot (تجربة الاستضافة الذاتية)** ✅ مكتمل (2026-05-22)
- `v0.5.9` — **Phase 4.5 Handoff Bundle (حزمة تسليم الإقلاع)** ✅ مكتمل (2026-05-22)
- `v0.9.0.1` — **Stage-0 Bootstrap Manifest (بيان إقلاع المرحلة صفر)** ✅ مكتمل (2026-05-22)
- `v0.9.0.2` — **Mixed C+Baa Harness (حاضنة C+Baa المختلطة)** ✅ مكتمل (2026-05-23)
- `v0.9.1` — **Lexer Rewrite Checkpoint (نقطة فحص إعادة كتابة المحلل اللفظي)** ✅ مكتمل (2026-05-29)
- `v0.9.1.4` — **Systems Ergonomics for Baa Lexer (تحسينات نظم لقراءة كود المحلل اللفظي)** ✅ مكتمل (2026-05-29)
- `v0.9.1.4.5` — **Lexer Transition Kit (حزمة انتقال المحلل اللفظي)** ✅ مكتمل (2026-05-29)
- `v0.9.1.5` — **Fully Move Lexer to Baa (نقل المحلل اللفظي كاملاً إلى باء)** 📝 قيد التنفيذ (2026-05-29)
...
| **المعالج القبلي** | `#تعريف`، `#إذا_عرف`، `#إذا_لم_يعرف`، `#الغاء_تعريف` |
...
| **تشخيص عربي موجّه (v0.5.4)** | نطاقات مؤشر أدق، وأسطر `مساعدة:` عربية للأخطاء الشائعة، واختبارات سلبية تقيس التشخيصات المتتالية |
...
| **مجموعة الإقلاع Baa0 (v0.5.7)** | `docs/BAA0_SPEC.md` و`scripts/qa_bootstrap_gate.py` يحددان السطح المحافظ لأول نقل ذاتي |
| **تجربة استضافة ذاتية (v0.5.8)** | `scripts/qa_selfhost_pilot.py` يربط وحدة Baa0 صغيرة مع C ويقارنها بسلوك الواجهة الحالية |
| **حزمة تسليم Phase 4.5 (v0.5.9)** | `scripts/qa_phase45_handoff.py` يجمع أدلة QA والبيانات الحتمية وحالة الجاهزية قبل v0.9 |
| **بيان Stage-0 (v0.9.0.1)** | `docs/STAGE0_TOOLCHAIN_MANIFEST.json` و`scripts/qa_stage0_manifest.py` يثبتان لقطة الإقلاع قبل نقل وحدات C إلى Baa |
| **حاضنة C+Baa المختلطة (v0.9.0.2)** | `scripts/qa_mixed_harness.py` يربط شرائح Baa0 مع C ويثبت لقطات تدفق وحدات lexer |
| **نقطة فحص المحلل اللفظي (v0.9.1)** | `src/frontend/lexer_state_baa0.baa` يمارس مسار Baa المباشر في حاضنة تدفق الوحدات مع لقطات تشخيص lexer/preprocessor |
| **تحسينات نظم للمحلل اللفظي (v0.9.1.4)** | دعم `هيكل* -> عضو` و`&` لقيم `هيكل`/`اتحاد` القابلة للإسناد لتقليل كود الحالة الخام عند نقل lexer إلى باء |
| **حزمة انتقال المحلل اللفظي (v0.9.1.4.5)** | دعم `خام"..."` كـ `ط٨*` ودوال `طول_خام/قارن_خام/قارن_خام_بطول` مع هدف mixed-harness جديد لاستهلاكها في Baa0 |
| **حالة lexer مملوكة لباء (v0.9.1.5)** | `src/frontend/lexer_state_baa0.baa` يملك موضع/سطر/عمود الماسح ويعيد بيانات رموز بسيطة إلى حاضنة C+Baa عبر ABI ضيق |
...
1) لا توجد بعد صيغة `extern` مستقلة للمتغيرات العامة، لذا الواجهة المشتركة المستقرة بين الملفات هي واجهات الدوال.
