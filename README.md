...
المسار الحالي هو نقل المصرّف نفسه إلى باء خطوة خطوة. العمل النشط الآن في `v0.9.2` هو نقل المحلل القواعدي بعد اكتمال مسار lexer:

- يوجد المحلل اللفظي الإنتاجي في `src/frontend/lexer.baa`.
- يوجد عقد ترويسة باء له في `src/frontend/lexer.baahd`.
- يوجد غلاف parser الإنتاجي في `src/frontend/parser.baa` لمسار المستوى الأعلى، مع بقاء قواعد التصريحات مؤقتاً في C.
- تبقى واجهات التوافق C مثل `lexer.h` و `parser.h` ثابتة أثناء النقل حتى لا تتغير عقود المكونات اللاحقة.
