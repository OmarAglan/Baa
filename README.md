...
| **المؤشرات (v0.3.10)** | تعريف مؤشرات عامة + `&` (عنوان) + `*` (فك الإشارة) + `عدم` كمؤشر فارغ + إسناد عبر المؤشر `*م = قيمة` + حساب مؤشري (`pointer +/- int` و`pointer - pointer`)؛ `ثابت T*` يثبّت متغير المؤشر لا القيمة المشار إليها |
