...
- [x] **Keep C compatibility wrapper** — the existing `lexer_init`/`lexer_next_token`/cleanup contract remains parser-facing while the implementation can still be selected at build time.
- [x] **Opt-in production Baa-backed lexer wrapper** — `BAA_USE_BAA_LEXER=ON` routes the existing parser-facing C lexer API through the Baa scanner while preserving token heap ownership, source/include buffers, dependency arrays, diagnostics, BOM handling, and cleanup behavior.
- [x] **Default lexer flip** — the normal `baa` build now uses the Baa-backed lexer by default, with an internal C-lexer `baa_stage0` bootstrap target and `-DBAA_USE_BAA_LEXER=OFF` rollback.
