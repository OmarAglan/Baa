...
* \[x] **Compiler-owned inlay hints** — `--inlay-hints=json` emits deterministic `inlay-hints-json-v1` Arabic parameter labels from resolved call bindings, suppresses self-explanatory variable arguments, and marks recovered or truncated output incomplete without asking tooling to parse Baa source.
