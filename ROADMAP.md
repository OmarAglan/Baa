...
* \[x] **Selective safety flags** — `-fruntime-checks=<list>` now accepts `all`, `bounds`, `null`, `div-zero`/`div0`/`div`, `shift`, and `none` with comma or plus separators.
...
* \[x] **Source-level shadow matrix** — classify all 100 sources on both targets as Arabic-only emitted, visibly unsupported, or gate error; every rejection carries a stable Arabic blocker kind/detail. The integer/control and Arabic symbol expansion covers 8/16/32/64-bit registers, saved frames, memory moves, arithmetic, shifts, comparisons, `setcc`, extensions, division, direct/indirect calls, function addresses, and local branches, admitting 14 sources and rejecting 86 with status `3` and no output.
