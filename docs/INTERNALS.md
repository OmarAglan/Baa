...
| **6. Assemble** | `.s` Assembly | `.o` Object | `gcc -c` | Invokes external assembler. On Windows (v0.4.4.1+), toolchain calls run via process-unique ASCII staging paths, then outputs are copied back to requested UTF-8 paths. |
| **7. Link** | `.o` Object | `.exe` Executable | `gcc` | Links with C Runtime. On Windows (v0.4.4.1+), link inputs/outputs are staged on process-unique ASCII paths for GCC compatibility. |
...
The Parser (`src/frontend/parser.c` plus the Baa slice in `src/frontend/parser.baa`) builds the AST using Recursive Descent with 2-token lookahead. The v0.9.2 migration contract starts at `src/frontend/parser.baahd`, which exposes a raw-handle Baa parser entrypoint returning `عقدة_باء` from `src/frontend/ast.baahd`. The production C-facing API remains `parse(Lexer*) -> Node*`, and it now routes through the Baa wrapper by default. Baa owns the top-level declaration-list loop, declaration dispatch/classification, the top-level type-alias declaration shell, the top-level `ثابت` / `ساكن` declaration qualifier loop, the unexpected top-level declaration recovery shell, the top-level non-alias declaration prefix shell, the compound-definition versus named-symbol tail dispatch after a parsed declaration type prefix, and the function/array/global-variable tail classification for named top-level declarations; detailed declaration tail bodies, reusable type-spec parsing, statement parsing, expression parsing, alias registry storage, AST allocation, and diagnostics internals remain C-owned until later parser-migration slices.
