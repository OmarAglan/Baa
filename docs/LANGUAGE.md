...
- لا يوجد تحقق حدود افتراضي لفهرسة النص، لكن `-fruntime-checks` يضيف حارساً اختيارياً لـ `نص[i]` وفهرسة محارف عناصر `نص[]`؛ عند الفشل يطبع `فهرس خارج حدود النص` ثم ينهي بـ `exit(1)`.
...
- يمكن تفعيل فحوص وقت التشغيل الاختيارية عبر `-fruntime-checks`: فحوص حدود للفهرسة غير الثابتة في المصفوفات والنصوص، وفحوص مؤشر فارغ قبل فك الإشارة، وفحوص قسمة/باقي قسمة على صفر، وفحوص عدد الإزاحة حيث يستطيع الخافض إصدارها.
...
- عند تفعيل `-fruntime-checks` يضيف الخافض حارساً اختيارياً قبل فك مؤشر متغير أو الإسناد عبره؛ عند الفشل يطبع `فشل_مؤشر_فارغ` ثم ينهي بـ `exit(1)`.
...
عند تفعيل `-fruntime-checks` يضيف الخافض حارساً اختيارياً قبل القسمة الصحيحة وباقي القسمة؛ عند القسمة على صفر يطبع `فشل_قسمة_على_صفر` ثم ينهي بـ `exit(1)`.
...
- قيم الإزاحة الصريحة يجب أن تكون بين `٠` و`٦٣`، ومع `-fruntime-checks` تُحرس قيم الإزاحة الديناميكية أيضاً؛ عند الفشل يطبع البرنامج `فشل_إزاحة_غير_صالحة` ثم ينهي بـ `exit(1)`.
...
يمكن تضمين أسطر تجميع خام داخل الدالة عبر `مجمع`.
...
**القيود المدعومة حالياً (v0.4.0.6):**
...
- سطور `مجمع` يجب أن تكون نصوصاً ثابتة.
- تعبيرات المخارج يجب أن تكون قابلة للإسناد (مثل متغير، عنصر مصفوفة، فك مؤشر).
- معاملات `مجمع` تدعم القيم الصحيحة والمؤشرات فقط في هذا الإصدار.
- دعم القيود المتقدمة (مثل clobbers/فئات سجلات أوسع) مؤجل لإصدار لاحق.
