...
- التحويلات العددية الصريحة مدعومة بين الأنواع العددية (`int/float/char`).
- تحويلات المؤشرات الصريحة مدعومة بين `pointer <-> pointer` و`pointer <-> int`.
- التحويل الصريح إلى `عدم` غير مدعوم.
- التحويل الصريح إلى الأنواع المركبة (`هيكل`/`اتحاد`) غير مدعوم.
