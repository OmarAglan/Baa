...
- [x] **Multi-line Comment Handling**: Robust handling for multi-line comments (`/* ... */`) is implemented across all preprocessor stages. They are correctly stripped and do not interfere with parsing or directive recognition. The preprocessor maintains comment state across line boundaries.
