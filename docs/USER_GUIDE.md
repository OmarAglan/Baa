...
|------|-------------|---------|
| `<input.baa>` | Source file to compile. | `.\baa.exe main.baa` |
...
| `-S` | **Compile only to Assembly.** Produces `.s` file, does not invoke assembler/linker. | `.\baa.exe -S hello.baa` (creates `hello.s`) |
