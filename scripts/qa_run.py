...
def _run_example_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "example-compile-tests",
        [sys.executable, str(TESTS_DIR / "test_examples.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=120.0,
    )
...
def _run_header_self_check_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "header-self-check-tests",
        [sys.executable, str(TESTS_DIR / "test_header_self_check.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=120.0,
    )
...
def _run_tokens_json_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "tokens-json-tests",
        [sys.executable, str(TESTS_DIR / "test_tokens_json.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=120.0,
    )
...
def _run_target_info_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "target-info-tests",
        [sys.executable, str(TESTS_DIR / "test_target_info.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=120.0,
    )
...
def _run_nazm_emitter_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "nazm-emitter-tests",
        [sys.executable, str(TESTS_DIR / "test_nazm_emitter.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=120.0,
    )
...
def _run_version_sync_guard(log_dir: Path) -> StepResult:
    return _run_logged(
        "version-sync",
...
def _run_version_sync_tests(log_dir: Path) -> StepResult:
    return _run_logged(
        "version-sync-tests",
        [sys.executable, str(TESTS_DIR / "test_version_sync.py")],
        cwd=ROOT,
        log_dir=log_dir,
        timeout_s=MODULE_SIZE_TIMEOUT_S,
    )
...
def main() -> int:
    ...
    version_sync_test_res = _run_version_sync_tests(log_dir)
    _print_step(version_sync_test_res)
    all_results.append(version_sync_test_res)
    overall_ok = overall_ok and version_sync_test_res.passed
    if not version_sync_test_res.passed:
        return _write_summary(args.mode, baa, overall_ok, all_results, log_dir, args.summary_json)

    version_sync_res = _run_version_sync_guard(log_dir)
    _print_step(version_sync_res)
    ...
    example_res = _run_example_tests(log_dir)
    ...
    header_self_check_res = _run_header_self_check_tests(log_dir)
    ...
    tokens_json_res = _run_tokens_json_tests(log_dir)
    ...
    all_results.append(nazm_emitter_res)
    overall_ok = overall_ok and nazm_emitter_res.passed
