...
static bool parser_baa_decl_prefix_take(ParserBaaDeclPrefix* prefix, ParserBaaDeclParts* out)
{
    ...
    out->qualifiers = prefix->qualifiers;
    out->tok_type = prefix->tok_type;
    out->type = prefix->type;
    out->type_name = prefix->type_name;
    out->ptr_base_type = prefix->ptr_base_type;
    out->ptr_base_type_name = prefix->ptr_base_type_name;
    out->ptr_depth = prefix->ptr_depth;
    out->func_sig = prefix->func_sig;
...
static bool parser_baa_decl_prefix_is_compound_definition(const ParserBaaDeclPrefix* prefix)
{
    if (!prefix) return false;

    return (prefix->type == TYPE_ENUM || prefix->type == TYPE_STRUCT ||
            prefix->type == TYPE_UNION) &&
...
static Node* parse_declaration_after_prefix(ParserBaaDeclPrefix* prefix)
{
    if (parser_baa_decl_prefix_is_compound_definition(prefix)) {
        return parse_compound_declaration_after_prefix(prefix);
    }
...
int64_t محلل_قواعد_باء_تصريح_بنوع_تعريف_مركب(ParserBaaDeclPrefix* prefix)
{
    return parser_baa_decl_prefix_is_compound_definition(prefix) ? 1 : 0;
}

Node* محلل_قواعد_باء_تصريح_تعريف_مركب_أنهِ(ParserBaaDeclPrefix* prefix)
{
    return parse_compound_declaration_after_prefix(prefix);
}
