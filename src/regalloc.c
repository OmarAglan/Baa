...
const char* phys_reg_name(PhysReg reg) {
    if (reg >= 0 && reg < PHYS_REG_COUNT) return phys_reg_names[reg];
    if (reg == PHYS_NONE) return "none";
    return "???";
}
...
bool phys_reg_is_callee_saved(PhysReg reg) {
    ...
    switch (reg) {
        case PHYS_RBX: case PHYS_RBP: case PHYS_RSI: case PHYS_RDI:
        case PHYS_R12: case PHYS_R13: case PHYS_R14: case PHYS_R15:
            return true;
        default:
            return false;
    }
...
static const PhysReg alloc_order[] = {
    ...
    PHYS_RAX, PHYS_RCX, PHYS_RDX, PHYS_R8, PHYS_R9,
};
...
RegAllocCtx* regalloc_ctx_new(MachineFunc* func) {
    ...
    if (ctx->max_vreg < 1) ctx->max_vreg = 1;
...
static void record_use(BlockLiveness* bl, int vreg, int max_vreg, int words) {
    if (vreg < 0 || vreg >= max_vreg) return;
...
void regalloc_compute_liveness(RegAllocCtx* ctx) {
    ...
    if (!block_arr) { free(temp); return; }

    int bi = 0;
    for (MachineBlock* b = ctx->func->blocks; b; b = b->next) {
        if (bi < bc) block_arr[bi] = b;
        bi++;
    }
...
static void update_interval(RegAllocCtx* ctx, int vreg, int pos) {
    if (vreg < 0 || vreg >= ctx->max_vreg) return;
...
void regalloc_linear_scan(RegAllocCtx* ctx) {
    if (!ctx || ctx->interval_count == 0) return;
...
static PhysReg resolve_special_vreg(int vreg) {
    switch (vreg) {
        case -1:  return PHYS_RBP;
        case -2:  return PHYS_RAX;
        case -10: return PHYS_RCX;
        case -11: return PHYS_RDX;
        case -12: return PHYS_R8;
        case -13: return PHYS_R9;
        default:  return PHYS_NONE;
    }
}
...
void regalloc_insert_spill_code(RegAllocCtx* ctx) {
    if (!ctx || !ctx->func || ctx->spill_count == 0) return;
...
bool regalloc_func(MachineFunc* func) {
    ...
    if (func->block_count == 0) return true;
